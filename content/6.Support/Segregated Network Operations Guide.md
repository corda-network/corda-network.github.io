...
Customer fills in the SN requirements form and submits it as a Task ticket in Corda Network Support Portal. 
...
Information sent by R3 Support (including Customer’s unique Network Map URL) allows Customer to configure their notary service identity and notary worker nodes. Consult this page for a technical overview and this page for step-by step business process.

Please note that while the Doorman details will be the same as in the onboarding workflow, the Network 
Map URL is unique to each SN environment.

To ensure that the CSR requests for notary service identity and notary worker nodes will not have any errors, it is advised that Customer submits proposed X500 name for pre-approval over the same ticket raised in Step 1. Instructions on how to fill in the fields of an X500 certificate can be found in the Corda Network Rulebook.
...
Change management is covered in greater detail in the Corda Network Maintenance and Service Level Handbook.
4. Tasks
4.1	Requesting network parameter change
To request a Network Parameters Update,  Customer should raise a ticket via the Corda Network Support Portal,   outlining new network parameters, giving at least 3-weeks' notice i.e. 15 business days. 
...
Updates to some network parameters can be configured to be auto acceptable i.e. the new parameters will be accepted without user input. Auto acceptable parameters include epoch, whitelistedContractImplementations, modifiedTime and packageOwnership. 
...
More information on Network Parameter Update process is available online.
4.4 Requesting Certificate Revocation

To request any Participant certificate to be revoked Customer should raise a ticket via Support Portal, providing the following information:

•	A sound business reason to justify permanent removal of the X.500 certificate from the segregated network, such as a change of the company legal name, compromised private key, or the Participant decides to permanently withdraw from the Network.
...
•	Specify the X.500 name of the certificate to be revoked.
...
Materials on how to set up and run the health survey tool can be found here. Further information on node monitoring and logging is available here.
